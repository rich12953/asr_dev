...
final class Directory extends Renderer
{
    ...
    {
        $templateName = $this->templatePath . ($this->hasBranchCoverage ? 'directory_branch.src' : 'directory.src');
    ...
    private function renderItem(Node $node, bool $total = false): string
    {
        ...
        if ($total) {
            ...
            if ($node instanceof DirectoryNode) {
                ...
                $data['name'] = sprintf(
                    '%s <a class="small" href="%s.src">[line]</a> <a class="small" href="%s_branch.src">[branch]</a> <a class="small" href="%s_path.src">[path]</a>',
                    $node->name(),
                    $node->name(),
                    $node->name(),
                    $node->name()
        ...
        $templateName = $this->templatePath . ($this->hasBranchCoverage ? 'directory_item_branch.src' : 'directory_item.src');
