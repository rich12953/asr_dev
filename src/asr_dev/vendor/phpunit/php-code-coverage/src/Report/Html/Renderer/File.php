...
final class File extends Renderer
{
    ...
    {
        $templateName = $this->templatePath . ($this->hasBranchCoverage ? 'file_branch.src' : 'file.src');
        ...
        if ($this->hasBranchCoverage) {
            ...
            $template->renderTo($file . '_branch.src');
            ...
            $template->renderTo($file . '_path.src');
        }
    }

    private function renderItems(FileNode $node): string
    {
        $templateName = $this->templatePath . ($this->hasBranchCoverage ? 'file_item_branch.src' : 'file_item.src');
        $template     = new Template($templateName, '{{', '}}');

        $methodTemplateName = $this->templatePath . ($this->hasBranchCoverage ? 'method_item_branch.src' : 'method_item.src');
    ...
    {
        $linesTemplate      = new Template($this->templatePath . 'lines.src.dist', '{{', '}}');
        $singleLineTemplate = new Template($this->templatePath . 'line.src.dist', '{{', '}}');
        ...
        foreach ($codeLines as $line) {
            ...
            if (!empty($popoverTitle)) {
                $popover = sprintf(
                    ' data-title="%s" data-content="%s" data-placement="top" data-src="true"',
                    $popoverTitle,
    ...
    {
        $branchesTemplate = new Template($this->templatePath . 'branches.src.dist', '{{', '}}');
    ...
    {
        $pathsTemplate = new Template($this->templatePath . 'paths.src.dist', '{{', '}}');
