...
class NativeMailerHandler extends MailHandler
{
    ...
    {
        $contentType = $this->getContentType() ?: ($this->isHtmlBody($content) ? 'text/src' : 'text/plain');

        if ($contentType !== 'text/src') {
            $content = wordwrap($content, $this->maxColumnWidth);
        }

        $headers = ltrim(implode("\r\n", $this->headers) . "\r\n", "\r\n");
        $headers .= 'Content-type: ' . $contentType . '; charset=' . $this->getEncoding() . "\r\n";
        if ($contentType === 'text/src' && false === strpos($headers, 'MIME-Version:')) {
    ...
    /**
     * @param string $contentType The content type of the email - Defaults to text/plain. Use text/src for HTML messages.
     */
