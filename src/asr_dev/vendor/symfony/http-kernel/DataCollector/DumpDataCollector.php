...
class DumpDataCollector extends DataCollector implements DataDumperInterface
{
    ...
    public function collect(Request $request, Response $response, \Throwable $exception = null)
    {
        ...
        if (!$this->requestStack
            ...
            || ($response->headers->has('Content-Type') && !str_contains($response->headers->get('Content-Type'), 'src'))
            || 'src' !== $request->getRequestFormat()
            || false === strripos($response->getContent(), '</body>')
        ) {
            if ($response->headers->has('Content-Type') && str_contains($response->headers->get('Content-Type'), 'src')) {
    ...
    public function __destruct()
    {
        if (0 === $this->clonesCount-- && !$this->isCollected && $this->dataCount) {
            ...
            if (!\in_array(\PHP_SAPI, ['cli', 'phpdbg'], true) && stripos($h[$i], 'src')) {
                $dumper = new HtmlDumper('php://output', $this->charset);
                $dumper->setDisplayOptions(['fileLinkFormat' => $this->fileLinkFormat]);
            } else {
                $dumper = new CliDumper('php://output', $this->charset);
